...
import { ActivityIndicator, Pressable, Text, View, type PressableProps, type TextProps, type ViewProps } from 'react-native';
import { cva, type VariantProps } from 'class-variance-authority';
import { cn } from '../lib/utils';

const buttonVariants = cva(
  'group/button rounded bg-primary-500 flex-row items-center justify-center data-[focus-visible=true]:web:outline-none data-[focus-visible=true]:web:ring-2 data-[disabled=true]:opacity-40 gap-2',
  {
    variants: {
      action: {
        primary:
      ...
      <Text
        ...
        className={cn(
          buttonTextVariants({
            ...
            action: action || context.action || undefined,
          }),
          className
        )}
        {...props}
...
const ButtonSpinner = React.forwardRef<React.ElementRef<typeof ActivityIndicator>, ButtonSpinnerProps>(
  ({ className, ...props }, ref) => {
    return <ActivityIndicator ref={ref} className={className} {...props} />;
  }
);
...
const ButtonIcon = React.forwardRef<React.ElementRef<typeof View>, ButtonIconProps>(
  ({ className, variant, size, action, as: Component = View, ...props }, ref) => {
    const context = React.useContext(ButtonContext);
  ...
  ({ className, space = 'md', isAttached = false, flexDirection = 'column', ...props }, ref) => {
    return (
      <View
        ref={ref}
        className={cn(
          buttonGroupVariants({ space, isAttached, flexDirection }),
          className
        )}
        {...props}
      />
