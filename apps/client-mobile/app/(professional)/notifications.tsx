...
const DEFAULT_CONTENT = {
  ...
  'empty.body': "You'll see job requests and messages here.",
} as const;

export default function ProfessionalNotificationsScreen() {
  const router = useRouter();
  const { user } = useAuthStore();
  const content = useAppContent('professional_notifications', DEFAULT_CONTENT);
  ...
  const {
    ...
    refetch,
    isRefetching,
  } = useNotifications(user?.id || '', 'handy');
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <Header
        ...
        onBackPress={() => goBackOrReplace(router, '/(professional)/(tabs)/dashboard')}
      />
