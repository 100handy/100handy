"use client";

import Image from "next/image";
import { Check } from "lucide-react";

export function TaskerHero(): React.JSX.Element {
  return (
    <section className="bg-white py-16">
      <div className="mx-auto max-w-[1920px] px-8">
        <div className="grid items-start gap-12 lg:grid-cols-[1.2fr_1fr]">
          {/* Left side - Phone mockup with terracotta background */}
          <div className="relative flex justify-center lg:justify-start">
            <div className="relative w-fit rounded-3xl bg-brand-terracotta px-16 py-20">
              {/* Phone Frame */}
              <div className="relative w-[400px] h-[540px]">
                <Image
                  src="/images/phone-mockup-new.png"
                  alt="Phone Frame"
                  width={644}
                  height={868}
                  className="absolute inset-0 h-full w-full object-contain"
                />

                {/* Phone Screen Content */}
                <div className="absolute inset-[8%_15%_12%_15%] bg-[#3A3A3C] rounded-[35px] overflow-hidden">
                  {/* App Icon */}
                  <div className="absolute top-[15%] left-[10%] w-20 h-20 bg-white rounded-2xl flex items-center justify-center">
                    <div className="text-center">
                      <div className="text-[11px] font-bold text-brand-dark-alt leading-tight">100<br/>HANDY</div>
                      <div className="bg-brand-terracotta text-white text-xs font-bold px-2 py-1 rounded mt-1">Task</div>
                    </div>
                  </div>

                  {/* App Title */}
                  <div className="absolute top-[15%] left-[40%]">
                    <p className="text-white text-base font-medium">100 Handy Task</p>
                    <p className="text-gray-400 text-xs">By 100 Handy</p>
                  </div>

                  {/* Ratings Section */}
                  <div className="absolute top-[30%] left-[10%] right-[10%] flex items-center justify-between">
                    {/* Stars */}
                    <div className="flex gap-1">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className="w-4 h-4 text-gray-600" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                        </svg>
                      ))}
                    </div>
                    <span className="text-gray-500 text-lg font-medium">5+</span>
                    <span className="text-gray-500 text-lg font-medium">#72</span>
                  </div>

                  {/* Download Button */}
                  <div className="absolute top-[45%] left-[10%] right-[10%]">
                    <button className="w-full bg-[#A0B194] text-white text-xl font-bold py-4 rounded-xl">
                      Download
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>

          {/* Right side - Account Created info */}
          <div className="flex flex-col justify-start pt-4">
            <div className="mb-8 flex items-center gap-2">
              <div className="flex h-6 w-6 items-center justify-center rounded-full bg-brand-sage">
                <Check className="h-3.5 w-3.5 text-white" strokeWidth={3} />
              </div>
              <h1 className="text-[45px] font-bold leading-none text-brand-dark-alt">
                Account Created
              </h1>
            </div>

            <h2 className="mb-6 text-[35px] font-bold text-brand-dark-alt">
              Next Steps:
            </h2>

            <div className="space-y-4">
              <p className="text-[23px] font-medium leading-relaxed text-brand-dark-alt">
                1. Download the tasker app and<br />
                sign in to your account.
              </p>

              <p className="text-[23px] font-medium leading-relaxed text-brand-dark-alt">
                2. Complete and submit your<br />
                profile for verification.
              </p>

              <p className="text-[23px] font-medium leading-relaxed text-brand-dark-alt">
                3. Once it's approved, start earning<br />
                money your way!
              </p>
            </div>

            {/* App Store Badges */}
            <div className="mt-8 flex gap-3">
              <a href="#" className="block">
                <div className="flex h-14 items-center gap-3 rounded-lg bg-black px-4">
                  <svg
                    className="h-7 w-7"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    color="white"
                  >
                    <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                  </svg>
                  <div className="text-left">
                    <div className="text-[10px] text-white">Download on the</div>
                    <div className="text-sm font-semibold text-white">App Store</div>
                  </div>
                </div>
              </a>
              <a href="#" className="block">
                <div className="flex h-14 items-center gap-3 rounded-lg bg-black px-4">
                  <svg
                    className="h-7 w-7"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    color="white"
                  >
                    <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z" />
                  </svg>
                  <div className="text-left">
                    <div className="text-[10px] text-white">GET IT ON</div>
                    <div className="text-sm font-semibold text-white">Google Play</div>
                  </div>
                </div>
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
