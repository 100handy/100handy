...
export interface MonthlyHandyStarItem {
  user_id: string
...
export function useMonthlyHandyStars(month: string) {
  return useQuery({
    queryKey: ['handys', 'stars', month],
    queryFn: async (): Promise<MonthlyHandyStarItem[]> => {
      ...
      if (!fiveStarReviews || fiveStarReviews.length === 0) {
        return []
      }
      ...
      const leaderboard = (profiles || []).map((profile) => {
        ...
        return {
          ...
          average_rating: ratings.count > 0 ? Math.round((ratings.sum / ratings.count) * 10) / 10 : (profile.rating || 0),
